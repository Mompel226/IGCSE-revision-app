...
* Where the answer is one of a few, the cell is a **dropdown** — the class list,
  `complete`/`progress`.
...
* **Muted by default, strong colour only where something needs looking at.** Percentages get
  ...
  hand-in and a score under 40% are the loudest things on screen. One typeface, one size,
  no gridlines — banding and spacing carry the structure, with a hairline where one group of
  columns ends and the next begins.
* Tabs are colour-coded along the bottom: the dashboard and lab tabs green, Labs amber,
  Rejected red.

**Importing a class does all of this for you** — it creates any tab that is missing, gives every
new class its place on the dashboard, refreshes it and dresses the lot. There is
