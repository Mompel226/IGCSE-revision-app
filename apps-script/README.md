...
One Google Sheet collects every Biology Lab. A tab per lab, your Classroom rosters
beside them, and the switch that closes Mastery during a test — all from one script,
one deployment, one URL pasted into every lab.

You do not need to know any Apps Script. Make a Sheet, paste two files in, run
`setup`, deploy. Five minutes, once.
...
| Tab | What is in it |
|---|---|
| **Setup** | what everything is, and the web app URL, in plain words |
| **Labs** | one row per lab: its id, its tab, **Mastery open** ☑, the message students see when it is closed, and a live count of hand-ins |
| **Students** | your Classroom rosters — name, school email, class, which course they came from |
| **Digestion**, **Circulation**, … | one tab per lab: every hand-in, newest at the bottom |
| **Summary** | best % per student per lab, one row each, rebuilt from the menu |

Every tab is styled by the script, not by hand: a dark-green frozen header, columns
wide enough for what is actually in them, long text wrapped rather than cut off, the
name column frozen so it stays put when you scroll right, alternating row shading, a
filter row on every sheet, and a red-amber-green scale on the percentages. Change
anything you like — **🧪 Biology Labs ▸ Re-apply the formatting** puts it back.

**Filtering to one class** is the filter button on the *Class* column of any tab.
The rosters use the same class codes as the hand-ins, so `9A` means the same thing
everywhere.
...
3. **Extensions ▸ Apps Script.** Delete what is there and paste in
   [`Code.gs`](Code.gs) (also below). Put the ID into `SHEET_ID` at the top.
4. **Add the dialog.** In the editor: **+** next to *Files* ▸ **HTML** ▸ name it exactly
   `ClassroomImport` ▸ paste in [`ClassroomImport.html`](ClassroomImport.html) (also below). Save.
5. **Add the Classroom service.** **Services (+)** ▸ **Classroom** ▸ **Add**.
6. **Run ▸ `setup`.** Authorise when asked — it is your own script on your own Sheet.
   It builds and styles every tab.
7. **Deploy ▸ New deployment ▸ Web app**, *Execute as* **Me**, *Who has access*
   **Anyone**. Deploy, and copy the `/exec` URL.
8. **Paste that URL** into each lab's `js/config.js` as `submitUrl`, commit, push.
   The same URL for every lab — the lab tells the script which one it is.

> **After any edit to the script:** Deploy ▸ Manage deployments ▸ pencil ▸
> *Version: New version* ▸ Deploy. Editing alone changes nothing; the old version
> keeps serving until you redeploy.

---

## The three things you will actually do

**Import your classes.** 🧪 Biology Labs ▸ *Import students from Classroom…* It lists
your active courses, guesses a class code from each course name, and shows how many of
that class you already have. Tick, check the codes, Import. Each class ticks green as it
finishes. **Closing the window does not stop it** — the import runs on Google's side and
writes its result whether you are watching or not.

Run it again whenever someone joins. Students already in the sheet are skipped, never
duplicated — the key is the school email — and anyone whose class has changed is moved.

**Close Mastery during a test.** Labs tab ▸ untick *Mastery open* for that lab. Every
open page follows within about two minutes, greys Mastery out and puts the student in
Test mode with the message in column E. Tick it again afterwards. Or, for all labs at
once, the menu has *Close Mastery everywhere*.

**Read the results.** Each lab tab holds a row per hand-in:

| Column | What it tells you |
|---|---|
| When · Name · Class · Mode | who, and whether it was a Mastery run or a Test |
...
| Working since | how long ago they first checked anything |
| Code · Code check · Flags | the completion code, whether it recomputes, anything odd |
| Per station | `mouth 8/8 in 11 · stomach 9/9 in 20` |

The last four columns are the point of Mastery. **113/113 · 214 checks · 71 right first
time · 3 days** is the evidence somebody ground it out; *Right first time* separates the
student who already knew it from the one who did the work.

---

## Putting marks into Google Classroom

Classroom only lets a script grade work that **the same script created** — an assignment
you made by hand in the Classroom UI cannot be graded through the API. So:

**A — no setup.** Set a Classroom assignment asking for the completion code. Students
paste theirs in; this sheet is the record beside it.

**B — marks pushed automatically.** From the Apps Script editor:

```javascript
createAssignmentFor('digestion-lab', 'YOUR_COURSE_ID')   // once — returns the coursework id
pushGradesFor('digestion-lab', 'YOUR_COURSE_ID', 'THE_COURSEWORK_ID')   // after a test
```

The course id is the long number in the Classroom address bar,
`classroom.google.com/c/`**`THIS BIT`**. `pushGradesFor` takes each student's best score
from that lab's tab and patches it into their submission, matching on name against the
class list. A name it cannot match is left alone and listed in the log — nothing is guessed.

---

## Honest limits

* The Mastery switch is a **classroom control, not security**. The labs are static sites;
  a determined student can work around anything the browser is told. It stops the ordinary
  case and makes the rule visible.
* If the sheet cannot be reached, a lab keeps the last answer it had, so a dropped
  connection never locks a class out of their homework.
* Names are matched to Classroom by name, not by email — the labs ask for a name and a
  class, not a login. Two students with the same name in one course need sorting by hand.
